...
def getContours(inputFrame, frameContour):                      #gibt auch simple

    contours, hierarchy = cv.findContours(inputFrame, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_NONE)
    #contours, hierarchy = cv.findContours(inputFrame, cv.RETR_TREE, cv.CHAIN_APPROX_NONE)   //TREE ist langsamer und ungenauer
    #contours, hierarchy = cv.findContours(inputFrame, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)    //chain macht keinen sind, karten sind rechteckig
